...
.. function:: decode(jwt, key="", algorithms=None, options=None, audience=None, issuer=None, leeway=0)
    ...
    :param dict options: extended decoding and validation options
        ...
        * ``verify_exp=verify_signature``  
          ...
          Check that ``exp`` (expiration) claim value is in the future
          * ``verify_iat=verify_signature``  
          
          Check that ``iat`` (issued at) claim value is an integer
        * ``verify_nbf=verify_signature``

          Check that ``nbf`` (not before) claim value is in the past

          
        .. warning::
            
            ``exp``, ``iat`` and ``nbf`` will only be verified if present.
            Please pass repsective value to ``require`` if you want make
            sure that they are always present (and therefore always verified
            assuming their respective ``verify_exp``, ``verify_iat``, and
            ``verify_nbf`` is set to ``True``).
